...
void Shape::init()
{
	...
	else
	{
		...
		glBufferData(GL_ARRAY_BUFFER, tanBuf.size() * sizeof(float), tanBuf.data(), GL_STATIC_DRAW);
	}

	// Send the binormal array to the GPU
	if (binormBuf.empty())
	{
		binormBufID = 0;
	}
	...
	{
		glGenBuffers(1, &binormBufID);
		glBindBuffer(GL_ARRAY_BUFFER, binormBufID);
		glBufferData(GL_ARRAY_BUFFER, binormBuf.size() * sizeof(float), binormBuf.data(), GL_STATIC_DRAW);
	}
...
{
	int h_pos, h_nor, h_tex, h_tan, h_bi;
	h_tan = h_bi = h_pos = h_nor = h_tex = -1;
	...
	h_bi = prog->getAttribute("vertBinorm");
	if (h_bi != -1 && binormBufID != 0)
	{
		GLSL::enableVertexAttribArray(h_bi);
		glBindBuffer(GL_ARRAY_BUFFER, binormBufID);
		glVertexAttribPointer(h_bi, 3, GL_FLOAT, GL_FALSE, 0, (const void *)0);
	}
	...
	if (h_bi != -1)
	{
		GLSL::disableVertexAttribArray(h_bi);
	}
	if (h_tan != -1)
	{
